...
marble is released. No red marble is ever released. Thus, the board accumulates blue marbles in its
...
\begin{tikzpicture}
  \begin{ttboard}[label={Turing Tumble}]
...
Above we saw how to make a board that just output blue marbles.\hint{Try thinking about making a
...
program to \emph{stop}. We can use the interceptor piece for this.
...
But now, when a blue marble comes out, it gets stuck in the interceptor and nothing happens! That's
...
of a way to make that work? \hint{Think of how we can replace the interceptor with another machine
  that does what we want!}

But can we make a board that
outputs an alternating color of marbles? If we want the board to do this, it would have to remember
what the last color was. This is where the bit element comes in.

In \prettyref{fig:alternate}, we can start by a marble of any color. If the bit starts out pointing
left, as in the figure, then, when a marble hits, it will flip to the right and send the marble
right. This will cause a \textcolor{marblered}{\emph{red}} marble to be released. The red marble
will again hit the bit, which will be pointing right. It will then flip left, and the marble sent to
the left, where it will eventually hit the blue lever to release a
\textcolor{marbleblue}{\emph{blue}} marble, and the cycle repeats.

No matter the color of the first marble, the second marble will \emph{always} be red if the bit
starts towards the left and always blue if the bit starts towards the right. That's a \emph{bit}
more interesting now, isn't it?

\begin{figure}
  \label{fig:alternate}
  \centering
  \begin{tikzpicture}
    \begin{ttboard}[label={Alternating Marbles}]
      \TTRamp[right]{3}{0}
      \TTRamp[right]{4}{1}
      \TTRamp[left]{7}{0}
      \TTRamp[left]{6}{1}
    ...
    \end{ttboard}
  \end{tikzpicture}
  \caption{A board that outputs alternating red and blue marbles. A bit is used to remember what the last marble released was.}
\end{figure}
