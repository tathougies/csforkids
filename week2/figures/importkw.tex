...
\begin{tikzpicture}[remember picture, overlay]
  ...
  \node[anchor=north] (star note) at ([yshift=-3em]import from star.south) {
    \begin{tcolorbox}[code note,width=8cm]
      ...
      then we could simply use rule names like \code{draw()} and \code{forward()}. We often keep
      modules named in case we ever have two rules with the same name.
    \end{tcolorbox}
  };
  ...
  \draw[->, ultra thick] ([yshift=0.1ex]star note.north) to[in=-90,out=90] ([yshift=-0.2em]import star mid);
\end{tikzpicture}
