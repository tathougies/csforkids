...
\begin{table*}[t]
  ...
  \begin{tabular}{p{0.24\linewidth}p{0.76\linewidth}}
    ...
    \code{draw.darken(col)} / \code{draw.lighten(col)} & Take a \code{col} and give a new color that is slightly darker or lighter than the original color. \\
    \code{draw.blend(col1, col2)} & Blend \code{col1} and \code{col2} together to form a new color between the two. \\
...
\didyouknow{The first computers
...
work does not match what actually happens, we call it a \term{bug}.
...
\tikzforeground{
  ...
  \node[anchor=\tikztuftemarg] (immediate note hint) at ([xshift={\tikztuftetowardsmargin * 0.7cm},yshift=0.2cm]current page text area.\tikztuftemain|-immediate note middle) {
    \begin{tcolorbox}[code note]
      You can set \code{immediate=True} to just get the final output, without having to watch Turtle draw.
    \end{tcolorbox}
  };

  \draw[->] (immediate note hint.\tikztuftemarg) to[out=\tikztufteangle{180},in=\tikztufteangle{-40},looseness=0.8] ($(immediate note target)!0.8!(immediate note target end)$);
}
\end{replbox}
...
\begin{TryThisBox}
  ...
  You can reference the file in \texttt{csforkids/week2/myduck.py} to get you started.
