...
def square():
  draw.draw(<ENTER>
    ...
    draw.forward(100) + draw.turn('right') +<ENTER>
    draw.forward(100) + draw.turn('right') +<ENTER>
    draw.forward(100) + draw.turn('right') +<ENTER>
    draw.forward(100)
  \end{lstlisting}
...
Now try this in the prompt.\hint{Remember to reload the file.}
...
because the \code{draw.draw()} rule always starts from a blank
screen. Because we put \code{draw.draw} in our \code{square} rule
...
Reload the file into the prompt, and let's try it.

\begin{replbox}
>>> draw.draw(square_shape() + draw.forward(200) + square_shape())<ENTER>
\end{replbox}

Uh-oh! That didn't work. What's going on?

Remember above how the \code{draw.forward()} was a \emph{plan} for a
drawing, but not the drawing itself? We were able to see the plan, by
just using \code{draw.forward()} without \code{draw.draw()}. Let's see
...
\begin{replbox}
>>> draw.draw(square_shape() + draw.forward(200) + square_shape())
\end{replbox}
...
def big_square_shape():
  return draw.color('blue') + \<ENTER>
    draw.forward(200) + draw.turn('right') + \<ENTER>
    draw.forward(200) + draw.turn('right') + \<ENTER>
    draw.forward(200) + draw.turn('right') + \<ENTER>
    draw.forward(200)
