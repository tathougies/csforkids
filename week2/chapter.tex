...
\tikzforeground{
  ...
  \node[anchor=east] (immediate note hint) at ([xshift={\tikztuftetowardsmargin * 0.7cm},yshift=-0.2cm]current page text area.\tikztuftemain|-immediate note middle) {
    \begin{tcolorbox}[code note]
      You can set \code{immediate=True} to just get the final output, without having to watch Turtle draw.
    \end{tcolorbox}
  };

  \draw[->] (immediate note hint.\tikztuftemarg) to[out=0,in=\tikztufteangle{-40},looseness=0.8] ($(immediate note target)!0.8!(immediate note target end)$);
}
\end{replbox}
