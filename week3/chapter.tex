...
\didyouknow{Remarkably, this number that was calculated thousands of years ago, is very accurate!
  You don't need to see much to be able to deduce important conclusions!}  Eratosthenes reasoned,
using geometry, that the angle of the shadow was the same as the angle from Alexandria, to the
center of the earth, and then back out again. He knew that there are 360 total degrees in any
circle, and that 7.5 degrees of that was 787.5 kilometers. Thus, the number that Eratosthenes
calculated is the circumference of the Earth.

\begin{figure}
  \hspace*{-4cm}
  \begin{tikzpicture}[scale=1,xshift=-5cm,font={\sffamily}]
    \node at (0, 8) {Sun};
    \draw (0, 8) circle (1);
    \draw (-2, 0) -- (10, 0) node[midway, anchor=north, yshift=-0.1em] {Ground};

    \draw (0,4) -- (0, 0) node[below]{\footnotesize Syene};

    \coordinate (alexandria bottom) at (6, 0);
    \coordinate (alexandria stick) at (6, 4);

    \node (degree mark) at ([yshift=1em, xshift=1em]alexandria stick) {$7.5^{\circ}$};
    \draw[->] (degree mark.south) to[right] ([yshift=-0.5em, xshift=0.2em]alexandria stick);
    ...
    \draw[ultra thick] (B) -- (alexandria bottom) node[below]{\footnotesize Alexandria};

    \node (shadow note) at ([yshift=1em, xshift=2em]B) {\footnotesize Shadow};
    \coordinate (shadow) at  ($(B)!0.5!(alexandria bottom)$);
    \draw[->] (shadow note.west) to[right] ([yshift=0.1em]shadow);

    \node[anchor=south east] (no shadow) at (-1, 1) {\footnotesize\begin{tcolorbox}[code note, width=3cm] No shadow because the Sun is directly above the stick\end{tcolorbox}};
    \draw[->] (no shadow.south) to[left] (-0.1,0.1);

    \draw[thick, decoration={brace, raise=0.2em}, decorate] (0.2,0.1) -- (5.8, 0.1) node[midway,yshift=1.5em] {787.5 kilometers};

  \end{tikzpicture}

  \caption{Eratosthenes's experimental setup. Eratosthenes noticed that on the summer solstice the
    shadows cast by the Sun at noon appeared different in two cities. In Syene, there was no shadow,
    which meant the Sun was directly overhead. In Alexandria, there was a shadow.}
...
\didyouknow{Polynesian navigators used principles similar to Eratosthenes to spread throughout the
  islands of Polynesia. Before mapmaking or even writing, they were able to sail thousands of miles
  across open aceans using only the stars and their oral knowledge of weather patterns. So great was
  their navigational prowess that when the earliest European explorers arrived in Polynesia,
  Polynesian navigators such as \textbf{Tupaia} were able to produce intricate maps like this one
  using only their memories of the stars they had observed on their many voyages.

  There's even reason to believe that some Polynesian navigators sailed across the Pacific and
  reached South America. You can learn a lot about big things by just looking at what's around you!

  \begin{center}
    \includegraphics[width=0.9\marginparwidth]{../week3/figures/polynesian-map.jpg}
  \end{center}
}
...
follow on their own without human input is called an \term{algorithm}.
...
\curious{The word \emph{algorithm} comes from the name of the Persian polymath and mathematician
...
\didyouknow{You can play DuckBot online at \resource[qrcode, text={DuckBot}, description={Play the
      DuckBot game online},
    kind={Resources}]{https://tathougies.github.io/csforkids/week3/demos/duckbot.html}.}  Oh and one
more thing: if DuckBot runs into any obstacle, like a rock, a plant, or the ground, DuckBot dies and
will never be able to make it to the ducklings. Your brain has to make sure he only ever swims in
water.

You can run DuckBot using Thonny. In Thonny, open the file
\texttt{csforkids/week3/game.py}.\hint{Don't be intimidated by the amount of code in this file. You
  won't have to change any of it. It's just the code behind DuckBot!} Then, click the green ``Run''
button.
...
told it. This is called its \emph{thought}. DuckBot can see what's around it in the current
square. Based off of this knowledge \emph{and} its previous thought, DuckBot has to decide the next
direction and its next thought.
...
\begin{minipage}{\textwidth}
  \Large \ttfamily
  \tikzforeground{
    \node[anchor=\tikztuftemarg](return comma note) at ([xshift={\tikztuftetowardsmargin * 0.5cm},yshift=1cm]current page text area.\tikztuftemain|-return comma.north) {
      \begin{tcolorbox}[code note, width=4cm]
        The \code{,} (comma) tells \kw{return} that we are going to send back more than one piece of data.
      \end{tcolorbox}
    };

    \draw[->] (return comma note.\tikztuftemarg) to[out={\tikztufteangle{-180}}, in=90] ([yshift=1.0em,xshift=0.5em]return comma.north);
  }
  \vspace{0.5cm}
  \begin{tcolorbox}[sharp corners]
...
string or even the \code{None} value.\hint{You'll probably want your thoughts to mostly be
  words. This will make your code easier to understand.} You get to decide!
...
DuckBot and the ducklings end up in random parts of the pond.\curious{Actually, the ducklings end up
  in the pond square you're least likely to end up in. That's why it's so important DuckBot hits the
  whole pond!}
...
\begin{figure}[b]
  \begin{minipage}{\linewidth}
    ...
    \vspace{3em}
    \tikzforeground{
      \coordinate (condition brace start) at ([yshift=1.7em, xshift=0.4em]condition start);
      \coordinate (condition brace end) at ([yshift=1.7em]condition end);
      \draw[thick, decoration={brace, raise=0.2em}, decorate] (condition brace start) -- (condition brace end) node[midway, yshift=0.3em, above]{
...
true of \kw{if}. If we want some code to run when the condition is true, then all this code must be
indented \emph{more} than the \kw{if} statement. Moreover, all the code has to line up. If you
indent one line with six spaces, and the next line with ten, then Python will return an error.

Let's try using \kw{if} to make DuckBot do something if we have an obstacle to the east. We will use
\kw{if} to check the data in \code{east}. After \kw{if}, we supply a rule that Python can use to
check if \code{east} has \code{'x'} or \code{'*'}.

\begin{TryThisBox}
  Change DuckBot's brain to this:
  \tikzforeground{
    \CheckTikzPictureSide
    \node[anchor=south west] (spaces duckbot note) at ([yshift=10em]spaces duckbot) {
      \begin{tcolorbox}[code note, width=5cm]
        This has \emph{four spaces} to place the \kw{return} beneath the \kw{if}. The other two
        lines, under the \kw{def}~\texttt{brain}, are indented only \emph{two spaces}.
      \end{tcolorbox}
    };
    \draw[->] (spaces duckbot note.west) to[out=180,in=180,looseness=0.8] ([yshift=1em]spaces duckbot.north);
  }
  \begin{lstlisting}
def brain(north, south, east, west, thought):
  @@if east == 'x':
  (*@\tikzmarknode{spaces duckbot}{}@*)  return 'N', thought@@
  return 'E', thought
  \end{lstlisting}
\end{TryThisBox}

\hint{Note that we use \emph{two} equal signs after an \kw{if}. The double equal
...
Uh-oh! This time DuckBot turned north when it hit an obstacle, but we never checked if there was an obstacle to the north, so it ran into the north wall.

Maybe it's a good idea to check if there's an obstacle to the north. But which direction to turn
then? North was left of east. We could go west, which is left of north.
...
This is an important part of algorithm design: you must handle \emph{all} cases. If you leave
certain situations out of your algorithm, then DuckBot will probably crash into one of the walls.

We can use \kw{elif} to write several conditions that are checked in order. \kw{elif} is written
like \kw{if}. You give it a condition. If the condition is true, then the code beneath it is
run. Finally, to supply code to run if none of the conditions are true, you can use \kw{else}. The
\kw{if} / \kw{elif}/ \kw{else} words need to be \emph{aligned} spacewise.

\begin{TryThisBox}
  \tikzmarknode{change duckbots brain}{Change} DuckBot's brain to this:
  \begin{lstlisting}[style=numbered, caption={A brain that checks for all obstacles},label={listing:duckbot-circle}]
def brain(north, south, east, west, thought):
  if east == 'x':
    return 'N', thought
  @@elif north == 'x':
  ...
  \end{lstlisting}
\end{TryThisBox}
\makemargmark{change duckbots brain}{\notestyle\hintnote This is one of those changes where it's really important that you try to \emph{predict} what
  is going to happen.}

If you go ahead and try that, you'll find that poor DuckBot \emph{still} hits the north
shore. Clearly we've not quite figured out how to make this work.
...
\begin{marginfigure}
  \centering
  \includegraphics[width=0.9\marginparwidth]{../week3/figures/duckbot-ne-desired.pdf}
...
that algorithm is realized in a particular programming language (Python) and in a particular
environment (the pond). For example, when we draw out the flowchart for the algorithm above (see
\prettyref{fig:flowchart}), we consider the idea of how the duck should move through the pond at a
global level. We don't think about how DuckBot makes its local decision. Instead we care about how
DuckBot decides how to move in order. Consider the node when the north is blocked and the east is
blocked. DuckBot goes west, and then thinks a new thought -- \code{'going\_west'} in this
case. Then, DuckBot goes south and thinks \code{'south'}. It stays in this state and continues south.

What did we leave out? We left out the \kw{if}s that inspected \code{thought}. This is because
\code{thought} is something that is particular to how DuckBot works on a square to square basis. The
flow chart captures what DuckBot ought to do globally. \hint{If you continue to draw flowcharts of
  how DuckBot should navigate all its level you will find that \code{thought} is fundamental to
  sequencing operations one after the other. Without it, you would only be able to decide based on
  the environment.} The \code{thought} is how we \term{implement} the flowchart as an algorithm for
DuckBot.
...
again. \prettyref{fig:flowchart-solution} corresponds to this algorithm.

\begin{figure}[p]
  \centering
  \includegraphics[width=\textwidth]{../week3/figures/flowchart1.pdf}
...
\begin{figure}[p]
  \centering
  \includegraphics[width=\textwidth]{../week3/figures/flowchart2.pdf}
  \caption{The flowchart corresponding to one way DuckBot can find the ducklings in the empty pond level.}
  \label{fig:flowchart-solution}
\end{figure}

\begin{TryThisBox}
  Try solving the empty pond again by directly translating from the flowchart in
  \prettyref{fig:flowchart-solution}. Remember that the diamond shape nodes correspond to \kw{if}
  and that sequences of multiple directions require you to create new \code{thought}s.
\end{TryThisBox}
...
\mTrackBox{Security}{Cyber-security is highly dependent on understanding how complex algorithms
  are. There are some algorithms that can encrypt data with a key quickly. Those who have the key
  can apply the algorithm in reverse to quickly find the original text. Meanwhile, those without the
  key have to use algorithms that can be proven to take longer than the age of the
  universe. Complexity theory is one of the most important branches of computer science to answer
  questions like these, with real world impact.}  These kinds of questions are an example of
thinking about \term{complexity}. There are multiple ways to study complexity and computer
programmers are interested in all of them. Thinking about how many rules DuckBot needs given a
particular map is one kind of complexity. Another kind of complexity is thinking about how quickly
DuckBot can cover any given map.  For example, consider an empty pond three squares tall and three
squares wide.  How many squares will DuckBot need to visit?

Measuring the complexity of algorithms is one of the many fields of research in computer science. It
may seem like just a fun logic puzzle, but actually, these research questions are the central
concern of fields like cyber-security, programming languages, and more.

\section{Conclusion: Thinking with Algorithms}

Algorithms require us to specify the exact sequence of steps that need to be followed based on the
data that we have been given. Even seemingly simple problems can be difficult to solve
algorithmically. Computers follow algorithms exactly as written, and this can lead to surprising
behavior that wouldn't make sense to us. \curious{The pond may seem simple, but the algorithms you
  wrote would be able to cover ponds of any size, even ones larger than you can comprehend. In fact,
  some of your algorithms can provably cover \emph{infinite} ponds!} Like Eratosthenes, DuckBot only
saw its immediate environment, but it was still able to navigate its world. The beauty of thinking
algorithmically is that you can understand large things while only having to think about a little at
a time.

\Exercises

\begin{exercises}
\item Go through all the DuckBot levels!

\item Given what you know about DuckBot, do you think DuckBot can cover any pond layout? Are there
  some layouts that DuckBot would not be able to cover? What changes could you make to the game play
  to allow DuckBot to cover any such map?
\end{exercises}
