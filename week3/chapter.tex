...
\begin{minipage}{\textwidth}
  \tikzforeground{
    ...
    \node[anchor=east] (brain function note) at ([yshift=0.5em, xshift={\tikztuftetowardsmargin * 1em}]current page text area.\tikztuftemain|-brain function name.north) {
      \begin{tcolorbox}[code note, width=4cm]
        Your function should be named \code{brain}.
      \end{tcolorbox}
    };
    \draw[->] (brain function note.east) to[out=0,in=90,looseness=0.8] ([yshift=1em]brain function name.north);
  }
  \Large \ttfamily
  \vspace{0.5cm}
...
\prettyref{fig:duckbot-desired-outcome}). That way it'll hit squares that are in the middle of the
...
\begin{TryThisBox}
  ...
  \hintnote One way to do this is to add another state that turns DuckBot completely around when it
  hits the southern wall.
\end{TryThisBox}
