...
Over the last few weeks, we've explored how use \emph{functions} to re-use pieces of rules. Last class, we discovered how to make decisions over  From now on, we'll
...
\begin{marginfigure}
\label{fig:speaker}
\includegraphics[width=\linewidth]{../week4/figures/loudspeaker.pdf}
\caption{
...
\begin{figure}[t]
  ...
  \label{fig:middle-a}
  \centering
  \input{../week4/figures/middle-a.tex}
\end{figure}
...
Let's begin by opening up \texttt{week4/synth.py} in Thonny.

The first thing we want to do is open up our waveform viewer. This will show us the actual
\term{samples} (speaker diaphragm offsets) being sent to the speakers. Click Run, and type the
...
\begin{replbox}
>>> view([note('A4'), note('A5')])<ENTER>
\end{replbox}

You can also supply frequencies directly:
\begin{replbox}
>>> view([note(440), note(880)])<ENTER>
\end{replbox}
...
\begin{TryThisBox}
  ...
  \texttt{week4/voices.py}
  \tikzforeground{
    \CheckTikzPictureSide
    \node[anchor=south west] (instrument decoration note) at ([yshift=-1em, xshift=14em]instrument decoration){
      \begin{tcolorbox}[code note, width=6cm]
        The line \code{@instrument} is called a \emph{decoration}, and helps the system realize that
        this function is an instrument. Without it, the instrument would not appear in our user
        interface.
      \end{tcolorbox}
    };

    \draw[->] (instrument decoration note.west) to[out=180,in=120,looseness=0.8] ([yshift=1em]instrument decoration.north);
    }
\begin{lstlisting}
(*@\tikzmarknode{instrument decoration}{}@*)@instrument
def example1(note, t):
  t1 = tone(note, t)
  t2 = tone(note * 2, t)
  t3 = tone(note * 3, t)
...
\mTrackBox{Music \& Sound}{Instruments make noise by vibrating air using strings, membranes, or open tubes. The
  ...
  \includegraphics[width=0.9\linewidth]{../week4/figures/standing-waves.eps}
  \attribution{MikeRun, CC BY-SA 4.0 <https://creativecommons.org/licenses/by-sa/4.0>, via Wikimedia Commons}
}

You can go ahead and play this instrument by hitting Run in Thonny. When you click run, a window
will pop up with the names of all instruments available. Select your instrument and a song from the
left pane, and hit Play.
...
\begin{figure}[p]
  ...
  \begin{subfigure}{0.9\textwidth}
    \includegraphics[width=\textwidth]{../week4/figures/tremolo-base.pdf}
    \subcaption{Base Wave}
    \label{subfig:tremolo-wave-basic}
  \end{subfigure}
  \hfill
  \begin{subfigure}{0.9\textwidth}
    \includegraphics[width=\textwidth]{../week4/figures/tremolo-transformed.pdf}
    \subcaption{Transformed}
    \label{subfig:tremolo-wave-transformed}
  \end{subfigure}
  \hfill
  \begin{subfigure}{0.9\textwidth}
    \includegraphics[width=\textwidth]{../week4/figures/tremolo-final.pdf}
    \subcaption{Transformed and shifted}
...
\prettyref{subfig:tremolo-wave-basic}. The wave peaks at 1 and
...
If we plot this (see \prettyref{subfig:tremolo-wave-transformed}), we see that the trough is now $-0.1$
...
Go ahead and try playing a song with the \texttt{tremolo} instrument. What does it sound like?
...
def funny_vibrato(note, t):
  vibrato = tone(6, t)
  return tone(note + vibrato * 0.1, t)
  \end{lstlisting}
\end{TryThisBox}

Now try using this instrument. What does it sound like?

Do you notice anything about this note? You may hear the vibrato become stronger and stronger
...
def vibrato(note, t):
  ...
  return @@tone_from_phase(note * t + vibrato * 0.1)@@
  \end{lstlisting}
\end{TryThisBox}
...
Now, try playing the \code{vibrato} tone.
...
\begin{table*}[t]
  ...
  \begin{tabular}{p{0.2\textwidth}p{0.7\textwidth}}
    \toprule
    \headercol{Function Name} & \headercol{Description} \\ \midrule
    \headercol{Tones} \\
    ...
    \code{piano\_attack\_env(t)} & An envelope suitable for something like an electric piano.\\
    ...
    \code{fade\_out(t)} & Produces a \emph{linear} fade-out effect starting at full volume at time 0 and ending in silence at time 1. \\
    \bottomrule
  \end{tabular}
  \vspace{1em}
  \caption{
    More functions you can use to manipulate
    instrument voices. Pay attention to the arguments to the
    functions. Tones produce sounds. Envelopes dynamically change
    volume. You can use an envelope with a tone by multiplying them
    together.}
  \label{tab:synths}
\end{table*}
...
Let's start with a simple ``real'' instrument.  \didyouknow{Fractions
  ...
  \includegraphics[width=0.9\linewidth]{../week4/figures/circle-of-fifths.pdf}
...
\hint{Try replacing the fade in \code{simple3} with a ``pluck'' fade by looking at
  \prettyref{tab:synths}. Does it sound different to you?}  In \code{simple3} we keep everything the
same, except we multiply the final wave by \code{fade\_out(t)}. This function starts at $1$ and then
goes to zero at time $t = 1$. There are different kinds of fade. Another fade is called an
\emph{exponential} fade, which fades out faster.
...
\caption{All our instrument voices can be represented as diagrams with arrows indicating how data flow through our functions, such as this one representing \code{simple5}.}
\centering
\scalebox{0.6}{
\input{../week4/figures/tone-hierarchy.tex}}
\end{figure}

\section{Conclusion: Building Blocks}

Using functions, we've been able to create some pretty complex voices. \term{Composition} is a
programming technique that allows us to combine behaviors in ways that produce ever more complicated
results. From a simple ``beep'' noise, we created all kinds of instruments. \curious{Try playing
  with all the different instruments in \texttt{week4.voices}} \code{Variables} helped us capture
data so that we could manipulate it before returning it from our function. Using variables, we
passed data from one function into another. Next week, we'll use a similar technique to go from
using a limited number of functions to using potentially \emph{infinite} numbers of functions! Stay
\emph{tuned} for more.
...
def phased(note, t):
  ...
  time = cycletime(note)
  return 0.5 * tone(note, t) + 0.5 * tone(note * 2, t + time * 0.5)
    \end{lstlisting}
