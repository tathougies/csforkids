...
\newrefformat{listing}{Listing~\ref{#1}\prettypageref{#1}}
...
\usetikzlibrary{calc,external,decorations.pathreplacing,arrows,arrows.meta,positioning,fit,tikzmark,ducks,backgrounds}
...
\makeatletter
\pgfkeys{
  /resourcegen/.is family, /resourcegen,
  name/.initial={},
  from/.initial={},
  type/.initial={},
  default/.style={
...
\newcommand{\CheckTikzPictureSide}{
  \checkoddpage
  \ifoddpage
  \def\tikztuftetowardsmargin{1}
  \def\tikztufteouter{east}
  \def\tikztufteinner{west}

  \def\tikztuftemain{east}
  \def\tikztuftemarg{west}
  \else
  \def\tikztuftetowardsmargin{-1}
  \def\tikztufteouter{west}
  \def\tikztufteinner{east}

  \def\tikztuftemain{west}
  \def\tikztuftemarg{east}
  \fi
}
\def\tikztufteangle#1{90 + (#1 - 90) * \tikztuftetowardsmargin}
%(\tikztuftetowardsmargin * (-90) + 90) + (\tikztuftetowardsmargin * #1)}
\makeatother
...
\newcommand{\codefamily}{\ttfamily\addfontfeatures{Ligatures=NoCommon,Contextuals=NoAlternate}}
...
\newtcbox{\code}{
  ...
  top=0pt,
  bottom=0pt,
  fontupper=\codefamily\footnotesize,
  nobeforeafter
}
...
\newcommand{\resourcegen}[1]{\
  \pgfkeys{/resourcegen, default, #1}
  \begingroup
  \protected@edef\resource@line{%
    GEN \theweek\space\thepage\space
    \pgfkeysvalueof{/resourcegen/name}\space
    \pgfkeysvalueof{/resourcegen/type}\space
    \pgfkeysvalueof{/resourcegen/from}\space
  }
...
\newcommand{\resource}[2][]{
  ...
  \def\resource@silence{\pgfkeysvalueof{/resource/silence}}
  \def\resource@files{\pgfkeysvalueof{/resource/files}}
  \def\tempa{}
  \begingroup
  \protected@edef\resource@line{%
    RESOURCE \theweek\space\thepage\space
    \resource@kind\space
  ...
  \immediate\write\weeks@dat{\resource@line}%
  \ifx\tempa\resource@files
  % Empty files
  \else
  \immediate\write\weeks@dat{FILES \resource@files}
  \fi
  \endgroup

  \ifnum\resource@silence=0
...
\newcommand{\week}[2][]{%
  ...
  \write\weeks@dat{DESCRIPTION \week@description}
}
\makeatother
...
\definecolor{lnbg}{gray}{0.88}   % background behind line numbers (darker gray)
\definecolor{lnfg}{gray}{0.25}   % line number text

\newcommand{\kw}[1]{\texttt{\textbf{\textcolor{CodeKeyword}{#1}}}}

\lstdefinestyle{kidcode}{
  backgroundcolor=\color{SoftGray},
  basicstyle=\codefamily\small,
  frame=single,
...
\lstdefinestyle{numbered}{
  style=kidcode,
  numbers=left,
  numberstyle=\sffamily\footnotesize\color{lnfg},
  stepnumber=1,
  numbersep=3pt,          % no gap between number gutter and code background
  xleftmargin=2.6em,        % we'll control gutter via framexleftmargin
  rulecolor=\color{lnfg}, % color of the left line
  framexleftmargin=2.6em, % width reserved for the number gutter (tweak)
}

\newcommand{\LstMetaFont}{\sffamily\footnotesize}
% \makeatletter
% \long\def\lst@makecaption#1#2{%
%   \vskip\abovecaptionskip
%   \sffamily
%   \sbox\@tempboxa{%
%     \LstMetaFont\color{CaptionLabelSlate}#1:\space
%     \LstMetaFont #2%
%   }%
%   \ifdim\wd\@tempboxa>\hsize
%     {\LstMetaFont\color{CaptionLabelSlate}#1:\space}{\LstMetaFont #2}\par
%   \else
...
\newcommand{\trythis}[1]{\marginnote{\notestyle\textbf{\textcolor{TryGreen}{Try This }}\color{black}#1}}
...
\input{../week5/chapter.tex}
\input{../week6/chapter.tex}
%\input{../week4/chapter.tex}
